...
class corpus:
	...
	def get_documents_pre_date(self,new_ID):
		...
		if new in self.documents_sorted:
			return set([d.ID for d in self.documents_sorted[:self.documents_sorted.index(new)]])
		else:
			return set([])

	def get_k_nearest_docuemnts(self,k,sim_func,new_ID, document_ID_set):
		# Finds the k nearest neigbors of the document new within document_set using
		# sim_func as the simiality measure
		sims = [] 
		for d_ID in document_set: 
			sims.append( (sim_func(self.documents[new_ID].TF,self.documents[d_ID].TF),d_ID) )
	...
	def find_influences(self,new_ID):
		...
		candidate_IDs = get_k_nearest_docuemnts(100,sp.spatial.distance.cosine,docs_predate_IDs)
